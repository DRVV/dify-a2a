...
def create_code_server():
    """Create and configure the code A2A server."""
    ...
    request_handler = DefaultRequestHandler(
        agent_executor=CodeAgentExecutor(dify_client),
...
if __name__ == "__main__":
    ...
    server, config = create_code_server()
    
    print(f"💻 Starting Code Agent at http://{config.host}:{config.port}")
