...
def create_analysis_server():
    """Create and configure the analysis A2A server."""
    ...
    request_handler = DefaultRequestHandler(
        agent_executor=AnalysisAgentExecutor(dify_client),
...
if __name__ == "__main__":
    ...
    server, config = create_analysis_server()
    
    print(f"📊 Starting Analysis Agent at http://{config.host}:{config.port}")
